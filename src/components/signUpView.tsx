...
export default class SignUpView extends React.Component<SignUpViewProps, SignUpViewState> {
    ...
    render(): React.ReactNode {
        ...
        <form onSubmit={this.onSignUp.bind(this)} target={"#"}>
            ...
            <input id="repeatedPassword" required name="repeatedPassword" placeholder="Repeat Password" className={ this.state.password != this.state.repeatedPassword ? "invalid" : "" }
                   type="password" onChange={(evt) => this.setState({ repeatedPassword: evt.target.value})} value={this.state.repeatedPassword} disabled={this.state.working} />
